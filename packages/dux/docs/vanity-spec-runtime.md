...
These app-plane helpers are imported from `@mszr/vanity/runtime`; their handle and snapshot types remain available from the package root for authoring declarations.

This is a direct CSS operation. It works with:

- vanity token custom-property handles;
...
- branded vanity values and bare numbers receive universal data-type checks; nonempty raw strings remain the standards/future-syntax lane and are ultimately parsed by CSS, while an optional schema may deliberately narrow them;
...
```ts
interface VanityRuntimeSnapshotV1 {
  version: 1
  system: string
  overrides: readonly {
...
For Nuxt, call `ds.runtimeProps(snapshot)` during SSR and bind its `style` map plus attributes on the effective root, then pass the same snapshot to `ds.runtime(root, { initial: snapshot })` in `onMounted`. The core API stays framework-neutral; the Nuxt runtime fixture locks this path before a later framework convenience is considered.
...
- Nuxt integration has an SSR-safe `runtimeProps()` path without making core framework-aware; framework conveniences must not change the snapshot contract.
...
Such a capability owns its sheet, replacement/deduplication, CSP diagnostics, shadow-root target, SSR limitations, and teardown. It never patches extracted vanity stylesheets.
