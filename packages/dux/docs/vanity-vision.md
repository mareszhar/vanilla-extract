...
> **vanity is a TypeScript harness for CSS.** It lets authors harness the full power of CSS through a language that feels native to TypeScript: inferred, composable, extensible, refactorable, inspectable, and delightful.
...
When vanity names a CSS concept, it should:
...
CSS is the floor, not the ceiling. `oklch()` must be as capable as CSS `oklch()`. A length constructor cannot make `'2em'` invalid. A strict property-alias policy cannot make standard CSS unreachable. New platform syntax cannot wait for a vanity release before users can employ it.
...
Vanity separates its work across three execution planes.
...
2. **Harness all of CSS.** CSS concepts retain platform capability and terminology. Vanity adds leverage; it does not create a smaller substitute language.
...
7. **Boring CSS is the contract.** Output remains inspectable, portable, overrideable, and useful if vanity disappears.
...
11. **Extensions are first-class.** Users can create the same kinds of values and utilities as vanity without subclassing private implementation types.
...
The CSS specifications supply canonical concepts such as custom properties, `var()`, data types, selectors, conditions, cascade layers, `@property`, and `@scope`. Vanity uses those names unless it introduces a genuinely higher-level design-system concept such as token, axis, mode, recipe, anatomy, or port.

Alignment does not mean freezing to today's grammar. CSS data types and functions evolve. Vanity maintains:
...
## 8. Output and override stance

- Non-atomic rules remain the default authoring/output model.
- Every vanity rule belongs to a declared system layer.
...
Vanity succeeds when users experience one coherent continuum:
...
This vision is the entry point to the complete contract:

- [Language](./vanity-language.md) defines the public vocabulary and canonical API shapes.
- [Patterns](./vanity-patterns.md) defines the cross-cutting rules every domain follows.
- [Decisions](./vanity-decisions.md) records settled architectural choices and their consequences.
- The specifications define [engines](./vanity-spec-engine.md), [values](./vanity-spec-values.md), [tokens](./vanity-spec-tokens.md), [CSS authoring](./vanity-spec-css.md), [runtime binding](./vanity-spec-runtime.md), [ports](./vanity-spec-ports.md), [recipes](./vanity-spec-recipes.md), [the preset](./vanity-spec-preset.md), [introspection](./vanity-spec-introspection.md), and [Vue/Nuxt integration](./vanity-spec-vue.md).
- [Testing](./vanity-testing.md) defines the evidence required for every product claim.
- [Demos](./vanity-demo.md) defines the flagship and comparison experiences without turning their current art direction into architecture.
- [Workspace](./vanity-workspace.md) documents the current repository, toolchain, and release mechanics.
- [Repository migration](./vanity-repository-migration.md) defines the move to Vanity's dedicated repository.
