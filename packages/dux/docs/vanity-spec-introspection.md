...
[vanity-patterns.md §11](./vanity-patterns.md#11-agent-legibility): agents and
...
The Vite plugin writes `.vanity/manifest.json`, refreshes it during development,
and serves the current projection at `/__vanity/manifest.json`. `/__vanity/`
renders the same data as a development browser.

`VanityManifest` is exported from `@mszr/vanity/vite`. Its version-2 contract
contains:
...
`@mszr/vanity/vite`. The implemented lanes are:
...
```TS
import { buildAgentContext, generateAgentContext } from '@mszr/vanity/vite'
```
...
[vanity-spec-tokens.md §14](./vanity-spec-tokens.md#14-dtcg-interchange).
Resolved export favors broad interoperability. Authored export preserves
vanity-specific graph/branch/plugin metadata in the versioned
`com.mszr.vanity` extension; plugins participate only through explicit,
stable codecs. A document exported in authored mode by vanity must import
...
[vanity-testing.md](./vanity-testing.md).
