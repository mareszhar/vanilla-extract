...
The framework overlays: where the delight investment goes, because Vue has never had a first-class typed build-time styling citizen. The core stays framework-free ([dux-vision.md §1.3](./dux-vision.md#13-vue-and-nuxt-are-the-first-clients-not-the-definition)); `/vue` and `/nuxt` are thin sugar over the core's three currencies — CSS, class strings, style-object fragments. Phase 5 of the roadmap.
...
- Accepts a thunk returning fragments (reactive) or plain fragments (static); the return binds to `:style` and serializes on the server ([dux-spec-ports.md §5](./dux-spec-ports.md#5-ssr)).
...
| `:deep(.child)` | piercing the scope wall to theme children | **ports** for values ([dux-spec-ports.md §4](./dux-spec-ports.md#4-child-and-consumer-theming)); `within()` for structure |
...
**Contract details.** This table ships in the package docs verbatim; the demo app exercises every row. Recipes bind through plain functions — no `useRecipe` composable exists, because a typed function needs no wrapper (principle 10).
