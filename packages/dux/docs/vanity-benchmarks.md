...
# vanity — benchmark baselines

This document records accepted performance baselines across implementation milestones. It is comparative evidence, not an API specification or a universal performance claim.
...
The ordinary gate timings are orientation numbers from one local run. The generated-scale metrics below are the accepted same-machine comparison baseline required by [testing](./vanity-testing.md#6-performance-benchmarks), not universal marketing claims.

## 3. Generated scale baseline

The source-controlled generator and fixtures live under `benchmarks/`; `pnpm run bench:fixtures:check` detects drift. `pnpm run bench:baseline` builds the SDK and writes the full protocol-1 result to ignored `.vanity/benchmarks/current.json`. This document commits the accepted human summary only.
...
The large fixture's instantiation and memory jump is the primary warning signal. Resolution-context and axis encodings are compared against this figure before becoming dependencies of public types.
...
```text
VANITY_DEFAULT_LAYERS, VanityError, alpha, aria, calc, channel, check,
...
| CSS output fixtures | Preserve valid value, cascade, layer, selector, and readability behavior. Exact private custom-property addresses may change when semantic output does not. |
| Manifest/introspection snapshots | Preserve complete provenance, semantic addresses, and traceability coverage. |
...
- public CSS/custom-property output remaining understandable without vanity.

### Replace as architecture/API

- color-specific rich IR plus minimal general `VanityCssValue` split;
...
The original comparison baseline includes:
...
The first series remains immutable as a comparison point. New accepted baselines append a dated series rather than rewriting earlier measurements.
