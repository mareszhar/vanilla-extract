...
Therefore a plain token graph emits inspectable custom properties and ordinary handle consumption uses `var()`. Graph edges are CSS-reactive by default: overriding an input custom property updates downstream expressions that have a CSS representation. This preserves vanity's public custom-property contract and keeps design tokens open to consumer CSS without per-leaf ceremony.
...
Vanity keys inside user structure use `$`:
...
interface VanityManifestToken {
  ...
  dependencies: readonly VanityManifestEdge[]
  support: {
    requirements: readonly string[]
    fallback?: string
    enhancement?: string
  }
  branches?: readonly VanityManifestBranch[]
  ...
  runtime?: VanityRuntimeTokenMetadata
  preview:
    | {
      status: 'resolved'
      val: string
...
interface VanityManifestDeclaration {
...
2. **Authored vanity document** — include `com.mszr.vanity` extension data for axes, cases, graph operations, emission, registration, and metadata.
...
- DTCG standard snapshot and vanity semantic round-trip fixtures;
