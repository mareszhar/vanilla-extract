...
The framework overlays are where Vue-specific delight belongs. The core stays framework-free ([vanity-vision.md §5](./vanity-vision.md#5-design-principles)); `/vue` and `/nuxt` are thin sugar over the core's three currencies: CSS, class strings, and style-object fragments.
...
- Accepts a thunk returning fragments (reactive) or plain fragments (static); the thunk's array **is** the merge — no `ports()` wrapper inside ([vanity-spec-ports.md §2](./vanity-spec-ports.md#2-setters-and-fragments)). The return binds to `:style` and serializes on the server ([vanity-spec-ports.md §6](./vanity-spec-ports.md#6-vue-ssr-and-hmr)).
...
- The call-site law applies unchanged: a wider props object flows through; unknown keys are ignored ([vanity-spec-recipes.md §4](./vanity-spec-recipes.md#4-the-call-site-props-in-classes-out)).
...
| `:deep(.child)` | piercing the scope wall to theme children | **ports** for values ([vanity-spec-ports.md §4](./vanity-spec-ports.md#4-published-component-styling-contracts)); typed class interpolation for structure ([vanity-spec-css.md §4](./vanity-spec-css.md#4-selectors-and-cross-file-references)) |
...
Nuxt application auto-imports are intentionally configured by the project. `vanityNuxtImports` is a convenience value for Nuxt's ordinary `imports.presets` option; importing and spreading it makes the choice visible. Projects may instead list a narrower preset or use explicit imports.

**Contract details.**

- Wires the `/vite` plugin (manifest emission included) and, when opted in, makes the configured system module available to evaluated `*.style.ts` files through the build-time inject shim. It does not force application auto-imports.
...
- **Mutable-token/mode flash:** persist application settings or `runtime.snapshot()` in a cookie/server payload, pass the snapshot through the exported `ds.runtimeProps()` projection when rendering the declared root, and bind `ds.runtime(root, { initial: snapshot })` on mount. The first SSR paint already contains the mode attributes and opaque slot values; hydration validates the same semantic addresses without rewriting them. The module's small `vanity-scheme` HTML-cookie adapter remains available for the common document-root light/dark case, but custom roots and multi-axis systems use their own bound runtime rather than a global theme registry.
...
- The sibling `*.style.ts` module is the stable, portable authoring form — required by the evaluation model ([vanity-patterns.md §1](./vanity-patterns.md#1-evaluate-dont-extract-compile-dont-run)).
- The sting is smallest exactly where colocation matters most: design-system styles (tokens, recipes, anatomies) *want* their own files, and small one-off styling stays short through `atoms` ([vanity-spec-preset.md §3](./vanity-spec-preset.md#3-atoms)) and `overrides`-layer `css()` calls.
