...
## 13. Naming map from the current implementation

| Current | Target |
| --- | --- |
| Root imports such as `defineTokens`, `oklch`, `scale` | Engine-derived `de.defineTokens`, `de.oklch`, `de.scale` |
| Root `createSystem()` | Canonical `de.createSystem()` |
| `.live()` | `token({ mutable: true })` for runtime mutation; `reference: 'var'` for cascade representation |
| `scheme({ light, dark })` | General engine axis with optional native scheme optimization |
...
| Internal `kind: 'parse'` | `literal` for recognized syntax; `raw` for deliberate opaque syntax |
