...
| --- | --- | --- |
| D01 | vanity is a **TypeScript harness for CSS**. | Same-named CSS concepts owe the platform's full capability; TypeScript adds leverage without narrowing CSS. |
| D02 | Delight is the governing product criterion. | Consistency with sibling projects is useful prior art, never authority. |
...
| D14 | Closed token config keys are unprefixed; vanity members sharing user namespaces use `$`. | `token({ val, axes, description })`; groups use `$axes`, `$description`, `$root`; handles use `$name`, `$val`, `$var`, `$axes`. |
...
| D37 | Runtime state can be snapshotted and projected to SSR root properties. | Persistence storage is app-owned; vanity owns validation, custom-property styles, runtime-managed mode attributes, hydration, and deterministic runtime schema identity. |
...
| D44 | DTCG import/export supports interoperable snapshots and vanity-authored extension data. | Vanity-to-vanity semantic round trips are lossless for portable nodes; arbitrary TypeScript source is not serializable. |
...
| D53 | The anti-mincho constraint remains an implementation law. | Every increment is an independently verifiable vertical slice, and the suite, demos, build, and packaging gates remain green at each boundary. Foundational work need not invent premature user-facing surface. |
| D54 | The value IR and public `createEngine()` share one internal engine kernel. | Root helpers delegate to the internal default engine, so constructors have one implementation and one identity model. |
...
| D60 | Var-default derivations may raise the CSS feature floor, never silently the supported-browser floor. | Engine target policy gates expression emission. Vanity emits a proven fallback/enhancement pair or diagnoses the unsupported dependency and suggests `reference: 'val'`; manifests/explanations include a resolved preview or an explicit preview-unavailable reason. |
...
| D62 | Resolution context uses separate `VanitySelfValue`/`VanitySystemValue` brands and a plane-neutral union, not a propagated public generic. | The 5,000-expression candidate generic cost 62% more TypeScript total time and 21.5% more declaration bytes than the branded/overloaded encoding. Focused overloads retain call-site rejection and readable hovers without taxing every value operation. |
| D63 | The flagship demo is a design-system studio, not a compatibility-preserved screen concept. | It proves one coherent system across live palette, scheme, density, radius, elevation, typography, motion, responsive/container behavior, and runtime persistence. The comparison demo uses a smaller parity-friendly brief; neither demo dictates product architecture. |
...
| D70 | Exact axes and cases accept one measured large-fixture TypeScript-memory exception. | At 5,000 tokens with 4 staged axes and representative cross-axis cases, TypeScript total time (2.23s), instantiations (2,216,611), declarations (445,681 B), and editor operations stayed within the accepted budgets, while reported memory rose from 414,338 kB to 573,904 kB. Exact axis, mode, and case completion is the user-visible gain; future work must monitor rather than compound this peak. |
...
| D74 | The measured runtime cost is the capability boundary, with application-output metrics locked for regression checks. | The browser runtime is 41,737 B / 7.53 kB gzip and the build-only root is 267,841 B / 46.8 kB gzip. This buys typed base, mode, and case setters; validation; semantic snapshots; SSR projection; hydration; reconciliation; and HMR binding without a framework or CSS-rule runtime. The 5,000-token app fixture emits 12,877 B / 2,775 B gzip JS; future work must not duplicate per-token runtime machinery. |
...
| D79 | Multi-source `propsOf.group()` uses object keys as namespace identity and hyphen-joins projected keys. | `propsOf.group({ button, card })` produces exact `button-intent`/`card-size` options; it also accepts already-projected option maps. No variable/export-name guessing or tuple-prefix drift exists. |
...
| D81 | Layout patterns return memoized classes; declaration utilities return plain fragments. | `stack`, `sidebar`, and peers remain patterns. `circle`, `square`, `truncate`, and `visuallyHidden` remain composable utilities. New additions require fixture-proven use cases and introduce no mixin runtime or new custom-property primitive. |
| D82 | Property aliases finalize typed engine configuration after axes and other plugins. | Exact both/aliases-only completion otherwise recursively instantiates the full CSS grammar through every later engine chain and exceeded the 4 GB compiler heap in the measured prototype. Installing the vocabulary policy last keeps ordinary engines at baseline cost; the resulting engine still exposes all constructors and `createSystem()`, and runtime normalization remains an ordinary public plugin contribution. |
...
| D84 | `com.mszr.vanity` authored interchange is version 1 and complements rather than masquerades as standard DTCG. | Resolved export emits only honest standard values for one environment. Authored export carries Vanity traits, branches/reservations, registrations, metadata, expressions, and portable dependency addresses. Plugin codecs declare stable `id`, `version`, extension identity, and JSON payloads; unknown DTCG extensions survive import/export; external resolution is explicit and synchronous. |
...
| D86 | Complete per-token semantics accept a measured manifest-artifact increase while preserving type, editor, and runtime budgets. | At 5,000 tokens manifest v2 is 6,265,577 B (150,057 B gzip) versus the prior 2,418,729 B artifact. This buys exact expression, inference, fold, support, declaration, branch, runtime, and portability evidence. One-pass reference indexes removed the initial quadratic build path; production generation is 4.66s. TypeScript total time is 2.05s, instantiations 2,217,790, declarations 447,372 B, CSS completion 2.70ms, runtime completion 0.11ms, and rename 5.18ms. The build-only root grows 17.1% to 323,889 B and runtime 2.0% to 43,528 B, both inside the 20% gate. |

## Deliberately open implementation details

These questions do not block the architecture, but must be settled in their owning contract before public implementation is considered complete:
...
- Whether an engine constructor-vocabulary policy should hide selected built-in helpers from completion so a project can prefer, for example, OKLCH-only color authoring. Such a policy may narrow helper exposure, never the CSS accepted by `css.standard`, `css.raw`, raw strings, or plugins.
