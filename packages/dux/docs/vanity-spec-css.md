...
Contracts here lean on the cross-cutting law: evaluation ([vanity-patterns.md §1](./vanity-patterns.md#1-evaluate-dont-extract-compile-dont-run)), the validation split ([§2](./vanity-patterns.md#2-type-the-names-parse-the-values)), conditions ([§5](./vanity-patterns.md#5-conditions)), layers ([§6](./vanity-patterns.md#6-layer-discipline)), and escape-hatch grace ([§8](./vanity-patterns.md#8-escape-hatch-grace)).
...
- **`tokens` accepts an engine-bound token module, and `t` is always returned** beside the authoring functions — one import line serves every style file. Token modules can stay inline or compose across files ([vanity-spec-tokens.md §1](./vanity-spec-tokens.md#1-definetokens--the-graph-in-plain-ts)).
...
- `de.createSystem` is itself evaluated build-time code; its returns are inert typed functions ([vanity-patterns.md §1](./vanity-patterns.md#1-evaluate-dont-extract-compile-dont-run)).
...
- Properties are csstype-typed camelCase; values accept tokens, ports, plain CSS values, color-helper expressions (`alpha(t.color.ink, 0.42)` — [vanity-spec-tokens.md §2](./vanity-spec-tokens.md#2-liveness-compilation)), and template interpolations of any of them.
- **`css()` is open-valued: any valid CSS value is legal with no escape wrapper.** The token map guides; it never gates. (`unsafe.value` exists only in `atoms`, whose property sets are deliberately closed — [vanity-spec-preset.md §3](./vanity-spec-preset.md#3-atoms). A utility-CSS refugee looking for "arbitrary value syntax" here should find this sentence: there isn't one, because everything is already allowed and still parsed.)
- Numbers take the property's canonical unit where one exists (`padding: 8` → `8px`; unitless properties like `lineHeight`, `opacity`, `zIndex`, `flexGrow` stay unitless). The number→unit table **is** the substrate's proven table — vanity delegates rather than re-deriving it — ships in the manifest, and is locked by the output test plane; it must never be a guess.
...
- Every utility returns a `VanityCssValue`: ordinary `toString()` plus a readable `.css` fact. Strings remain first-class and preferred wherever CSS is already the clearest spelling (`'system-ui'`, `'1px solid currentColor'`, or a template interpolation). The utilities are leverage, never ceremony.

---

## 3. Conditions

The behavior contract lives in [vanity-patterns.md §5](./vanity-patterns.md#5-conditions); this entry owns the authoring specifics.
...
- Interpolation is the *only* boundary-crossing form — deliberately. It looks like what it is (a selector reaching into a child), which keeps the honest hierarchy visible: theme a child's *values* through its ports ([vanity-patterns.md §4](./vanity-patterns.md#4-the-runtime-boundary-is-a-port)); select into a child's *structure* only when you own it, and the interpolated class says so in plain sight. A friendlier `within()` sugar was considered and deferred — a comfortable wrapper here would hand every `:deep()` refugee a crutch that delays learning ports ([vanity-vision.md §8](./vanity-vision.md#8-deferred-intentions)).
...
Per [vanity-patterns.md §6](./vanity-patterns.md#6-layer-discipline). Authoring specifics:
...
- The emitted stylesheet declares `@layer` order once, from the system config — app CSS outside vanity can slot into the same order by name.
- **Emission nests under the prefix.** Authoring says `layer: 'overrides'`; the CSS says `@layer vanity.overrides` (or `prism.overrides` under `prefix: 'prism'`). Cascade-layer order is *global* and first-declaration-wins, so bare names like `utilities` would couple a system to every coexisting framework's layer order (Tailwind and Panda both claim them); nesting means the only global name a system claims is its own namespace. When a coexisting framework layers aggressive resets (Tailwind's preflight), pin the order once, before any stylesheet loads: `@layer theme, base, components, utilities, vanity;` — the comparison sandbox demonstrates the whole dance.
...
**Contract details.** Same rule shape as `css()` including conditions; no class is generated. Global rules default to the `reset` layer and are enumerated by the escape audit when they target third-party selectors ([vanity-spec-introspection.md §3](./vanity-spec-introspection.md#3-audits)).
...
**Why.** The other half of "type the names, parse the values" ([vanity-patterns.md §2](./vanity-patterns.md#2-type-the-names-parse-the-values)): value grammar is beyond the type system's economic reach, so a real CSS parser validates every emitted rule at build.

**Contract details.**

- Every declaration — object, raw, global, keyframe step — is parsed; an invalid value is a build diagnostic with file:line, the offending property, and the reason (`VANITY_CSS_INVALID_VALUE`).
...
- **Setup failures are diagnosed too.** Importing a `*.style.ts` module without the `/vite` plugin registered produces one friendly error naming the missing plugin and the config line to add (`VANITY_VITE_PLUGIN_MISSING`) — never a raw Node evaluation stack. The bounce point of a misconfigured first install gets the same message quality as a typo'd property.
