...
The implemented [demo contract](./vanity-demo.md) governs the rebuilt Prism studio and dispatch-card comparison. Their visual concepts are replaceable, while the computed-style, accessibility, SSR, HMR, optimizer, and process assertions remain permanent gates.
...
| --- | --- | --- |
| 1 | Rename across modules and consumers | [`tokens.rename.test.ts`](../vanity/src/tokens/tokens.rename.test.ts) and canonical module IntelliSense in [`tokens.graph.dx.test.ts`](../vanity/src/tokens/tokens.graph.dx.test.ts). |
...
| 5 | Sparse multi-axis intersection | [`tokens.axes.out.test.ts`](../vanity/src/tokens/tokens.axes.out.test.ts) and Prism's scheme + density shadow explanation. |
| 6 | Override a published component contract | [`port.out.test.ts`](../vanity/src/ports/port.out.test.ts) and the port-driven Prism progress component. |
| 7 | Style typed headless states | [`css.dx.test.ts`](../vanity/src/css/css.dx.test.ts), recipe/anatomy fixtures, and keyboard-tested Prism tabs/dialog. |
| 8 | Bind reactive state through a typed port | [`port.test-d.ts`](../vanity/src/ports/port.test-d.ts), `usePorts` fixtures, and both live Prism progress bars. |
| 9 | Use CSS vanity does not yet model | [`css.out.test.ts`](../vanity/src/css/css.out.test.ts), Prism's scoped `@starting-style`, and the optimizer-survival gate. |
| 10 | Trace a rendered declaration | [`introspect.test.ts`](../vanity/src/introspect/introspect.test.ts) and Prism's build-produced `ds.explain()` provenance cards. |
| 11 | Export standard and authored DTCG | [`dtcg.test.ts`](../vanity/src/introspect/dtcg.test.ts), including resolved projection and portable authored round trips. |
| 12 | Extend values through the public engine | [`createEngine.test.ts`](../vanity/src/engine/createEngine.test.ts) and [`preset/plugins.test.ts`](../vanity/src/preset/plugins.test.ts); the flagship uses both public optional plugins. |
...
| 15 | Let an agent self-correct | [`introspect.dx.test.ts`](../vanity/src/introspect/introspect.dx.test.ts), [`audit.test.ts`](../vanity/src/introspect/audit.test.ts), and generated agent context from the same manifest records. |
