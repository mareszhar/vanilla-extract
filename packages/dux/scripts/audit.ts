/**
 * `pnpm run audit` — the introspection audits ([vanity-spec-introspection.md §3])
 * over a real plugin build. Defaults to the package's fixture app; point it at
 * any Vite-rooted style app: `pnpm run audit -- vanity/src/test-support/vite-app`.
 *
...
import { audit, formatAuditFindings } from '../vanity/src/introspect/audit'

const here = fileURLToPath(new URL('.', import.meta.url))
const packageDir = join(here, '..', 'vanity')
...
async function main(): Promise<void> {
  ...
  const { default: vanityPlugin } = await import('../vanity/src/vite')
