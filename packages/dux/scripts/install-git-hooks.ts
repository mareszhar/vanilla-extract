#!/usr/bin/env tsx
/**
 * Install the dux commit guard by pointing the repository's Git hook path at
 * `packages/dux/.githooks`.
 *
 * Why this script exists:
 * - `packages/dux` is intentionally not part of the outer vanilla-extract
 *   workspace, so the outer repo's hooks cannot be relied on to validate dux.
 * - Git has one `core.hooksPath` per worktree. Installing the dux hook there
 *   lets the hook inspect staged paths and no-op unless a commit touches dux.
...
const HOOKS_PATH = 'packages/dux/.githooks'
...
try {
  ...
  if (current !== HOOKS_PATH) {
    ...
    console.log(`[vane-dux] git hooks installed (core.hooksPath -> ${HOOKS_PATH})`)
  }
  else {
    console.log(`[vane-dux] git hooks already installed (core.hooksPath -> ${HOOKS_PATH})`)
  }
}
catch (error) {
  if (optional) {
  ...
  console.error(`[vane-dux] failed to install git hooks: ${message}`)
  process.exit(1)
}
