/**
 * Setup failures are diagnosed too ([vanity-spec-css.md §9]): an authoring call
...
export function requireStyleModule(surface: string): string {
  if (!hasFileScope()) {
    throw new VanityError({
      ...
      fix: 'add vanityPlugin() from \'@mszr/vanity/vite\' to vite plugins (or the \'@mszr/vanity/nuxt\' module), and keep this call inside a *.style.ts file',
    })
  }
