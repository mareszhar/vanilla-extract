/**
 * The public recipe and anatomy types ([vanity-spec-recipes.md]). Variants
 * compress state ([vanity-patterns.md §7]): the settled Stitches shape, kept
 ...
 *   ([vanity-patterns.md §10]) — a malformed options object reports a single
 *   diagnostic at the offending property.
 * - **Strict on literals, permissive on widened props** ([vanity-spec-recipes.md
...
import type { VanityNestedRule } from '../css/types'
 ...
 */
export type VanityRecipeArm<C extends string> = VanityNestedRule<C>
...
export interface VanityRecipeOptions<C extends string, L extends string, V, G, P> {
  /** Publication, not declaration: module-scope port handles become `button.ports.*` ([vanity-spec-recipes.md §2]). */
  ports?: P
...
> {
  /** Props in, classes out — unknown keys ignored, defaults fill the gaps ([vanity-spec-recipes.md §4]). */
  (props?: TProps): string
  /**
  ...
  /** The component's published runtime style API ([vanity-spec-recipes.md §2]). */
 ...
 * the system's conditions ([vanity-spec-recipes.md §3]).
 */
export type VanityAnatomyRule<C extends string, TPart extends string>
  = VanityNestedRule<C> & { [K in `${TPart}:${C}`]?: VanityNestedRule<C> }
...
export type VanityAnatomyArms<C extends string, TPart extends string> = {
  [K in TPart]?: VanityAnatomyRule<C, TPart>
}
...
export interface VanityAnatomyOptions<
  ...
  P,
> {
  /** The named parts, styled as one unit — *parts*, never "slots" ([vanity-language.md §3]). */
  parts: TParts
  ports?: P
 ...
 * resolution table — never a rule, never a stylesheet ([vanity-patterns.md §1]).
