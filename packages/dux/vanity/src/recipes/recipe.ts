/**
 * `recipe()` — variants compress state ([vanity-spec-recipes.md §1],
 * [vanity-patterns.md §7]). Per-arm emission plus a tiny lookup table: each
...
export function bindRecipe(system: VanitySystemContext) {
  return (options: VanityRecipeOptionsLoose, debugId?: string): VanityRecipe<Record<string, unknown>> => {
