/**
 * `anatomy()` — the recipe pattern applied to parts ([vanity-spec-recipes.md §3],
 * [vanity-patterns.md §7]). Same options grammar as `recipe` with one added
...
export function bindAnatomy(system: VanitySystemContext) {
  return (options: VanityAnatomyOptionsLoose, debugId?: string): VanityAnatomy<string, Record<string, unknown>> => {
 ...
 * ([vanity-spec-recipes.md §3]). Each of the condition's arms is re-anchored:
