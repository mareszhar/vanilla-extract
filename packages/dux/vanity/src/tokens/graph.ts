/**
 * `defineTokens` — the graph in plain TS ([vanity-spec-tokens.md §1]).
 *
...
type RuntimeContribution
  = { readonly kind: 'seed', readonly graph: VanityGraphInput, readonly emission: VanityTokenModuleOptions }
    | { readonly kind: 'derive', readonly stage: RuntimeStage, readonly emission: VanityTokenModuleOptions }
...
function buildTokens<T extends object, Prefix extends string = 'vanity'>(
  ...
  if (options.emitCss !== false) {
    for (const node of nodes.values()) {
      ...
      const axes: Record<string, Record<string, { value?: string | number, runtime?: import('../internal/handle').VanityHandleRuntimeAddress }>> = {}
      const cases: { when: Readonly<Record<string, string>>, value?: string | number, runtime?: import('../internal/handle').VanityHandleRuntimeAddress }[] = []
  ...
  const resolver: VanityResolver = {
    foldRef: (handle) => {
      ...
      if (definition.kind === 'literal' || definition.kind === 'value') {
        ...
        if (!parsed)
          throw new VanityError({ code: 'VANITY_TOKENS_INVALID_COLOR', message: `${node.key} holds '${css}', which is not a color`, path: node.key, file: graph.file })

        return parsed
      }

      if (definition.kind === 'none')
        throw new VanityError({ code: 'VANITY_TOKENS_INVALID_COLOR', message: `${node.key} has no authored default value`, path: node.key, file: graph.file })
...
/**
 * Record the resolved graph for the manifest ([vanity-spec-introspection.md §2]):
...
function resolveArm(root: string, arm: VanityAxisTriggerArm): {
