...
function applyChannel(current: number, operation: VanityColorChannel | VanityChannelOperation | undefined): number {
  if (operation === undefined)
    return current
  if (typeof operation === 'number')
    return operation
  if (operation === 'none' || !('kind' in operation))
    throw new TypeError('[vanity] a dynamic relative-color channel cannot be folded at build time')
