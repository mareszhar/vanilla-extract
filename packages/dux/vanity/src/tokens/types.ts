 ...
 *
 * - **Type the names** ([vanity-patterns.md §2]): token paths, emitted variable
 ...
 * - **The types are honest about liveness** ([vanity-patterns.md §3]):
...
type VanityConfigWithAxisDerivations<Config extends object, Axes extends VanityAxisDefinitions>
...
type VanityConfiguredAxes<Node> = Node extends VanityConfiguredToken<infer Config, any>
  ? Config extends { readonly axes: infer Axes } ? Axes : Record<never, never>
  : Record<never, never>

type VanityConfiguredCases<Node> = Node extends VanityConfiguredToken<infer Config, any>
  ? Config extends { readonly cases: infer Cases } ? Cases : readonly []
  : readonly []

type VanityConfiguredVal<Node> = Node extends VanityConfiguredToken<infer Config, any>
...
type VanityConfiguredReference<Node, Policy extends VanityTokenPolicy>
  = Node extends null ? 'var'
    : Node extends VanityConfiguredToken<infer Config, any>
...
type VanityConfiguredMutable<Node> = Node extends VanityConfiguredToken<infer Config, any>
  ? Config extends { readonly mutable: true } ? true : false
  : false

type VanityConfiguredDescription<Node> = Node extends VanityConfiguredToken<infer Config, any>
 ...
 */
type VanityCompositionGuard<A, B>
...
export type VanityCanonicalTokens<
  ...
  Policy extends VanityTokenPolicy = VanityDefaultTokenPolicy,
> = VanityResolvedTokens & VanityCanonicalTokenGroup<T, Name, '', Policy>
...
> = Node extends VanityDerived<infer Result>
  ? VanityTokenHandleOf<Result, Name, Path, Policy>
  : Node extends VanityLeafInput
    ? VanityTokenHandleOf<Node, Name, Path, Policy>
    : VanityCanonicalTokenGroup<Node, Name, Path, Policy>
...
> = VanityTokenHandle<
  ...
  Name,
  Path,
  Node extends VanityConfiguredToken<any, infer Type> ? Type : VanityDataTypeOf<VanityConfiguredVal<Node>>,
...
export type VanityNamesOf<Selection> = Selection extends VanityTokenHandle<any, infer Name, any, any, any, any, any, any, any, any>
  ? `--${Name}`
  : Selection extends object ? { readonly [K in keyof Selection]: VanityNamesOf<Selection[K]> } : never

export type VanityVarsOf<Selection> = Selection extends VanityTokenHandle<any, infer Name, any, any, any, any, any, any, any, any>
  ? `var(--${Name})`
  : Selection extends object ? { readonly [K in keyof Selection]: VanityVarsOf<Selection[K]> } : never
...
export interface VanityTokensOptions<T = unknown, Prefix extends string = string> {
  ...
  prefix?: Prefix
  /** Standalone guarantees over pairings the graph doesn't own ([vanity-spec-tokens.md §5]). */
...
export type VanityTokenOverrides<T> = {
  [K in keyof T]?: T[K] extends VanityTokenHandleAny
...
export type VanityThemeOverrides<T> = {
  [K in keyof T]?: T[K] extends VanityColorToken<any, any> | VanityContrastToken<any, any>
...
type VanityHasLive<N>
  = N extends VanityColorToken<infer M, any> ? (M extends 'live' ? true : false)
    : N extends VanityContrastToken<any, any> | VanityValueToken<any, any> ? false
      : N extends object ? (true extends VanityHasLive<N[keyof N]> ? true : false)
        : false
 ...
 * key, because writing it could not honestly work ([vanity-patterns.md §3]).
 */
export type VanityLiveOverrides<T> = {
  [K in keyof T as VanityHasLive<T[K]> extends true ? K : never]?:
