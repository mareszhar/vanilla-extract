 ...
 * `VanityPort` interface — the contracts of [vanity-spec-ports.md §1-3], asserted
 * at the type level.
 */

import type { VanityColorTokenHandle, VanityPort, VanityPortKind, VanityPortMeta, VanityPortSetValue, VanityVarReference } from '@mszr/vanity'
...
describe('set() typing', () => {
  it('a number port accepts a number or a reference', () => {
    ...
    expectTypeOf(fraction.set).parameter(0).toEqualTypeOf<VanityPortSetValue<'number'>>()
    fraction.set(0.62)
  ...
  it('a string port accepts a string or a reference', () => {
    ...
    expectTypeOf(width.set).parameter(0).toEqualTypeOf<VanityPortSetValue<'length'>>()
    width.set('8px')
