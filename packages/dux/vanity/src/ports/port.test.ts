 ...
 * of [vanity-spec-ports.md], asserted directly.
 */
...
describe('set()', () => {
  ...
  it('a color port set accepts a token reference — compiles to var()', () => {
    ...
    expect(returned.tint.set(returned.t.color.ink)).toEqual({ [returned.tint.name]: 'var(--vanity-color-ink)' })
  })
...
describe('restorePort', () => {
  it('rebuilds a port handle from serialized meta', () => {
    ...
    expect(handle.set(0.62)).toEqual({ '--vanity-fraction__h4x': 0.62 })
  })
  ...
  it('a restored branded port keeps serialized CSS values', () => {
    ...
    expect(handle.set('45deg')).toEqual({ '--vanity-angle__h4x': '45deg' })
  })
  ...
  it('a restored color port set accepts a string', () => {
    ...
    expect(handle.set('oklch(0.4 0.1 100)')).toEqual({ '--vanity-tint__h4x': 'oklch(0.4 0.1 100)' })
  })
  ...
  it('binds app-plane validators by stable id without global state', () => {
    ...
    expect(bound.set(0.5)).toEqual({ '--vanity-fraction__h4x': 0.5 })
