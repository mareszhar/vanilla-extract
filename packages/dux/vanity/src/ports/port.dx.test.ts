 ...
 * key, and hovers stay readable public types ([vanity-patterns.md §10]) — locked
...
describe('errors at the cursor', () => {
  it('a number port rejects a string in set()', () => {
    ...
    `
    expect(errors).toHaveError(/not assignable to parameter of type 'VanityPortSetValue<"number">'/)
  ...
  it('a string port rejects a number in set()', () => {
    const { errors } = project.check`${defineSystem}
      ...
      void width.set(8)
    `
    expect(errors).toHaveError(/not assignable to parameter of type 'VanityPortSetValue<"length">'/)
    expect(errors).toHaveErrorCount(1)
    expectNoLeak(errors)
  })
