 ...
 * ([vanity-patterns.md §2]) and the manifest format is a public type external
 * tools can build on.
 */
...
describe('the manifest format', () => {
  it('is versioned and shaped as documented', () => {
    expectTypeOf<VanityManifest['version']>().toEqualTypeOf<2>()
    expectTypeOf<VanityManifest['tokens']>().toEqualTypeOf<Record<string, VanityManifestToken>>()
