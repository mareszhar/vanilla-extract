/**
 * The manifest ([vanity-spec-introspection.md §2]): one machine-readable
...
import { createEngine, unsafe } from '@mszr/vanity'
...
describe('the manifest', () => {
  ...
  it('projects recipes: variant space, toggles, defaults, published ports', () => {
    ...
    expect(manifest.recipes.button.ports.fraction).toMatch(/^--vanity-fraction/)
  })
  ...
  it('projects ports under Component.export names, typed by their defaults', () => {
    ...
    expect(manifest.ports['prism.fraction'].var).toMatch(/^--vanity-fraction/)
  })
