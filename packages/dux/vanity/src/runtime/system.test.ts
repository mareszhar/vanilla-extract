...
describe('mutable runtime', () => {
  ...
  it('records runtime schema identity and opaque address provenance in inspection data', () => {
    ...
    expect(system).toMatchObject({
      kind: 'system',
      runtime: { protocol: 1, root: '#app', system: expect.stringMatching(/^vanity-runtime-1-/) },
    })
    ...
    expect(manifest.runtime?.system).toMatch(/^vanity-runtime-1-/)
  ...
  it('inspects semantic runtime overrides together with their concrete slot writes', () => {
    ...
    expect(runtime.inspect()).toMatchObject({
      system: expect.stringMatching(/^vanity-runtime-1-/),
      root: '#app',
      active: true,
      modes: { scheme: 'dark' },
      overrides: [{
