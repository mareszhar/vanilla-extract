/**
 * The vanity Vite plugin — evaluates `*.style.ts` at build time and emits
 * static CSS ([vanity-patterns.md §1], [vanity-workspace.md §3]). App code never
 ...
 * stable* virtual id (`/path/File.style.ts.vanity.css`) whose content is
 ...
 *
 * **The manifest rides the same evaluation** ([vanity-spec-introspection.md §2]):
...
export interface VanityViteOptions {
   ...
   * ([vanity-spec-vue.md §4]): an unbound `css` or `t` resolves to the system
...
/** `*.style.ts` (and variants) — vanity's authoring file extension. */
...
export function vanityPlugin(options: VanityViteOptions = {}): PluginOption[] {
  ...
  /**
   * Resolve the auto-import inject shim ([vanity-spec-vue.md §4]): a one-line
 ...
 */
function evaluateStyleModule(source: string, filePath: string, identOption: VanityIdentifierMode): EvaluatedStyleModule {
 ...
 * back to their export ([vanity-spec-ports.md §1], [vanity-spec-recipes.md §3]).
...
export type { VanityAuditFinding } from './introspect/audit'
