/**
 * The public atoms types ([vanity-spec-preset.md §3]): the strict utility lane.
 ...
 * property errors at that key ([vanity-patterns.md §2]).
 */
export type VanityAtomsPropertiesInput<P> = {
  [K in keyof P]: K extends VanityCssPropertyName ? VanityAtomValues : never
}
...
export type VanityAtomKey<V> = V extends readonly (infer U)[] ? U : keyof V & string

export type VanityAtomArms<V, C extends string>
  = { base?: VanityAtomKey<V> | VanityUnsafeValue } & { [K in C]?: VanityAtomKey<V> | VanityUnsafeValue }
