 ...
 * runtime atoms call never synthesizes CSS ([vanity-patterns.md §1]). Shared by
 ...
 * and it should be findable, reviewable, and removable ([vanity-patterns.md §8]).
 */
export const unsafe = {
  ...
  const pushValue = (classes: string[], property: string, condition: string, value: unknown): void => {
    if (isUnsafeValue(value)) {
      if (emitUnsafe) {
        ...
        + `runtime data crosses through a port, not the atoms lane ([vanity-patterns.md §4])`)
      }
