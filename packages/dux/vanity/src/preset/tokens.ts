/**
 * Preset tokens ([vanity-spec-preset.md §1]): a serious default palette, scale,
...
export type VanityPresetRadius = 'sharp' | 'calm' | 'round'
export type VanityPresetDensity = 'compact' | 'comfortable' | 'spacious'
export type VanityPresetContrast = 'soft' | 'balanced' | 'high'

/** A brand seed: a CSS color literal or an engine-authored color value. */
export type VanityPresetBrandInput = string | VanityAuthoredColor

export interface VanityPresetTokensOptions<
  B extends VanityPresetBrandInput = string,
  R extends VanityPresetRadius = 'calm',
> {
  ...
  density?: VanityPresetDensity
  /** How far inks and borders rise off the surfaces. `balanced` by default. */
  contrast?: VanityPresetContrast
}

export interface VanityPresetElevationOptions {
...
const densityUnits: Record<VanityPresetDensity, number> = {
  compact: 3,
  comfortable: 4,
  spacious: 5,
}

/** Elevation plane positions ([vanity-spec-tokens.md §4]) for the non-surface roles. */
const contrastPlanes: Record<VanityPresetContrast, { border: number, inkMuted: number, ink: number }> = {
...
export type VanityPresetBrand<_B extends VanityPresetBrandInput> = VanityAuthoredColor

/** The preset's definition graph — public because the builder carries it. */
export interface VanityPresetTokenGraph<B extends VanityPresetBrandInput, R extends VanityPresetRadius> {
  color: {
    brand: VanityConfiguredToken<{ readonly val: VanityPresetBrand<B>, readonly mutable: true }, 'color'>
    brandSoft: VanityDerived<VanityAuthoredColor>
    onBrand: VanityDerived<VanityContrast>
    canvas: VanityDerived<VanityAuthoredColor>
    surface: VanityDerived<VanityAuthoredColor>
    surfaceRaised: VanityDerived<VanityAuthoredColor>
    border: VanityDerived<VanityAuthoredColor>
    inkMuted: VanityDerived<VanityAuthoredColor>
    ink: VanityDerived<VanityAuthoredColor>
    brandHover: VanityDerived<VanityAuthoredColor>
    brandActive: VanityDerived<VanityAuthoredColor>
  }
...
export function presetTokens<
  B extends VanityPresetBrandInput = string,
  R extends VanityPresetRadius = 'calm',
  Policy extends VanityTokenPolicy = VanityTokenPolicy,
  Axes extends VanityAxisDefinitions = VanityAxisDefinitions,
>(
  engine: VanityCoreEngine<any, Policy, Axes>,
  options: VanityPresetTokensOptions<B, R> = {},
): VanityTokenModule<VanityPresetTokenGraph<B, R>, Policy> {
  const { alpha, defineTokens, legibleOn, mix, rawValue, scale, token } = engine
  const seed = options.brand ?? '#635bff'
  const brand = (typeof seed === 'string' ? rawValue.color(seed) : seed) as VanityPresetBrand<B>
  ...
  return builder as unknown as VanityTokenModule<VanityPresetTokenGraph<B, R>, Policy>
}
 ...
 */
function elevation(
  engine: Pick<VanityCanonicalCoreConstructors, 'mix' | 'oklch' | 'scheme'>,
  base: Parameters<VanityCanonicalCoreConstructors['mix']>[1],
  position: number,
  options: VanityPresetElevationOptions = {},
): VanityAuthoredColor {
...
function expectFactor(name: string, value: number): void {
  if (!Number.isFinite(value) || value < 0 || value > 1)
    throw new RangeError(`[vanity] elevation ${name} must be between 0 and 1; received ${value}`)
}
