/**
 * Layout patterns ([vanity-spec-preset.md §6]): the recurring compositional
...
type VanitySpaceValue = string | number | VanityVarReference | VanityTokenInput

export interface VanityPatternsConfig<Space extends Record<string, VanitySpaceValue>> {
...
type VanityAlign = 'start' | 'center' | 'end' | 'stretch' | 'baseline'
type VanityJustify = 'start' | 'center' | 'end' | 'space-between'

export interface VanityPatterns<SpaceKey extends string> {
  /** Vertical flow: `display: flex; flex-direction: column; gap`. */
  stack: (options?: { gap?: SpaceKey, align?: VanityAlign }) => string
  /** One row, no wrapping: `display: flex; align-items: center; gap`. */
  inline: (options?: { gap?: SpaceKey, align?: VanityAlign, justify?: VanityJustify }) => string
  /** A wrapping group: `display: flex; flex-wrap: wrap; gap`. */
  cluster: (options?: { gap?: SpaceKey, align?: VanityAlign, justify?: VanityJustify }) => string
 ...
 * ```
 */
export function definePatterns<Space extends Record<string, VanitySpaceValue>>(
  config: VanityPatternsConfig<Space>,
): VanityPatterns<keyof Space & string> {
