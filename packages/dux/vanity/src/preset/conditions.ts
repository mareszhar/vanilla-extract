/**
 * Preset conditions ([vanity-spec-preset.md §2]): the condition set where
 ...
 * facts, not opinions ([vanity-spec-css.md §1]); this map adds the opinionated
 * names on top.
 */

import type { VanityCondition } from '@mszr/vanity'

export interface VanityPresetConditionEngine {
  readonly container: (query: string) => VanityCondition
  readonly data: (name: string, value?: string) => VanityCondition
  readonly media: (query: string) => VanityCondition
}

export type VanityPresetConditionName
  = | 'sm' | 'md' | 'lg' | 'xl' | '2xl'
 ...
 * ([vanity-spec-recipes.md §5]).
 */
export function presetConditions(engine: VanityPresetConditionEngine): Record<VanityPresetConditionName, VanityCondition> {
