/**
 * Motion opinions ([vanity-spec-preset.md §5]): respecting
...
import type { VanityKeyframesRule } from '@mszr/vanity'

// ─── Named keyframe fragments — feed them to your system's `keyframes()` ─────

/** `const fadeIn = keyframes(fade)` */
export const fade: VanityKeyframesRule = {
  from: { opacity: 0 },
  to: { opacity: 1 },
}

export const slideUp: VanityKeyframesRule = {
  from: { opacity: 0, transform: 'translateY(12px)' },
  to: { opacity: 1, transform: 'translateY(0)' },
}

export const scaleIn: VanityKeyframesRule = {
