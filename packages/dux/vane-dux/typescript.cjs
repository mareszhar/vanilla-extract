/**
 * vane-dux TypeScript language-service plugin.
 *
  ...
  if (ts.isIdentifier(expression)) {
    ...
    for (const declaration of symbol?.declarations ?? []) {
      ...
      if (ts.isBindingElement(declaration) || ts.isParameter(declaration)) {
        ...
        if (call && ts.isCallExpression(call) && callName(ts, checker, call) === 'derive') {
...
function spanOf(node) {
  return { start: node.getStart(node.getSourceFile()), length: node.getWidth(node.getSourceFile()) }
}
