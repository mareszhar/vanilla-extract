/**
 * Conditions ([dux-patterns.md §5]): a named circumstance — pseudo, media,
...
export interface VaneConditionArm {
  media?: string
  supports?: string
  container?: string
  selector?: string
...
export interface VaneCondition {
  readonly arms: readonly VaneConditionArm[]
}

export type VaneConditionInput = string | VaneCondition

function condition(...arms: VaneConditionArm[]): VaneCondition {
  return { arms }
}
...
export function media(query: string): VaneCondition {
  return condition({ media: query })
}

/** A supports condition: `supportsAnchor: supports('(anchor-name: --a)')`. */
export function supports(query: string): VaneCondition {
 ...
 */
export function container(nameOrQuery: string, query?: string): VaneCondition {
  return condition({ container: query === undefined ? nameOrQuery : `${nameOrQuery} ${query}` })
}

/**
 * The effective scheme ([dux-spec-tokens.md §3]): the OS preference unless an
 * ancestor pins `data-scheme`. Two arms — the pinned subtree, and the
 * preference outside any opposite-pinned subtree.
 */
export function schemeIs(scheme: 'light' | 'dark'): VaneCondition {
...
export function data(attribute: string, value?: string): VaneCondition {
...
export function aria(attribute: string, value: string | boolean = true): VaneCondition {
  return condition({ selector: `&[aria-${kebab(attribute)}='${value}']` })
}

// ─── The base set ────────────────────────────────────────────────────────────

export type VaneBaseConditionName
  = | 'hover' | 'hoverFocus' | 'active' | 'focusVisible' | 'disabled'
    | 'motionOk' | 'motionReduce' | 'dark' | 'light' | 'ltr' | 'rtl'

/**
 * The built-in base conditions ([dux-spec-css.md §1]): the platform-universal
 ...
 * in the preset.
 */
export function baseConditions(): Record<VaneBaseConditionName, VaneConditionInput> {
  return {
    hover: '&:hover',
...
export function normalizeConditions(
  conditions: Record<string, VaneConditionInput>,
  file: string | undefined,
): Map<string, readonly VaneConditionArm[]> {
  const normalized = new Map<string, readonly VaneConditionArm[]>()
  const diagnostics: VaneDiagnostic[] = []

  for (const [name, input] of Object.entries(conditions)) {
    if (isCssProperty(kebab(name))) {
      diagnostics.push({
        code: 'VANE_SYSTEM_CONDITION_COLLISION' as const,
    ...
    for (const arm of arms) {
      ...
      if (reason !== undefined) {
        diagnostics.push({
          code: 'VANE_SYSTEM_INVALID_CONDITION' as const,
...
function parseConditionString(name: string, input: string, file: string | undefined): readonly VaneConditionArm[] {
  ...
  throw new VaneError({
    code: 'VANE_SYSTEM_INVALID_CONDITION',
