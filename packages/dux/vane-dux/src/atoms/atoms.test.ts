/**
 * The runtime plane for atoms ([dux-spec-preset.md §3]): token keys resolve to
...
describe('atoms resolution', () => {
  it('token keys resolve to precompiled classes', () => {
    ...
    expect(atoms({ gap: 'sm' })).toMatch(/^atoms_gap_sm__[\w-]+$/)
  ...
  it('condition maps pick the pre-generated arms', () => {
    ...
    expect(classes[0]).toMatch(/^atoms_padding_sm__[\w-]+$/)
    expect(classes[1]).toMatch(/^atoms_padding_md_md__[\w-]+$/)
  })
...
describe('the labeled escape', () => {
  it('emits at build time, memoized, with the label in the class', () => {
    ...
    expect(returned.first).toMatch(/^atoms_unsafe_inlineSize__[\w-]+$/)
...
describe('definition diagnostics', () => {
  it('an unknown condition dies at definition with the fix', () => {
    ...
    })).toThrowError(/VANE_ATOMS_UNKNOWN_CONDITION[\s\S]*did you mean 'md'/)
  })
