 ...
 * runtime atoms call never synthesizes CSS ([dux-patterns.md §1]). Shared by
...
import type { VaneAtoms, VaneAtomsRuntime, VaneUnsafeValue } from './types'

const UNSAFE = Symbol.for('vane.unsafeValue')
 ...
 * and it should be findable, reviewable, and removable ([dux-patterns.md §8]).
 */
export const unsafe = {
  value(value: string | number, reason: string): VaneUnsafeValue {
...
export function isUnsafeValue(value: unknown): value is VaneUnsafeValue {
 ...
 */
export type VaneUnsafeEmitter = (property: string, condition: string, escape: VaneUnsafeValue) => string

export function createAtomsHandle(runtime: VaneAtomsRuntime, emitUnsafe?: VaneUnsafeEmitter): VaneAtoms<Record<string, unknown>> {
  ...
  const pushValue = (classes: string[], property: string, condition: string, value: unknown): void => {
    if (isUnsafeValue(value)) {
      if (emitUnsafe) {
        ...
        + `runtime data crosses through a port, not the atoms lane ([dux-patterns.md §4])`)
      }
  ...
  return resolve as VaneAtoms<Record<string, unknown>>
}

function name(runtime: VaneAtomsRuntime): string {
