...
describe('the shared CSS value IR', () => {
  ...
  it('accepts typed expressions in individual color channels', () => {
    ...
    expect(defaultEngine.serialize(oklch(tokens.channel.lightness, tokens.channel.chroma, tokens.channel.hue)))
      .toBe('oklch(var(--vane-channel-lightness) var(--vane-channel-chroma) var(--vane-channel-hue))')
  })
  ...
  it('diagnoses feature requirements against an explicit project target', () => {
    const legacy = defineCssSupportTarget({ id: 'legacy-no-calc', features: [] })
    expect('add' in legacy.features).toBe(false)
    const serialize = createCssValueSerializer(legacy).serialize
    expect(() => serialize(calc('1rem').add('2px'))).toThrow(/calc-basic.*legacy-no-calc.*fallback/)
  ...
  it('publishes capability and maturity facts as data', () => {
    expect(VANE_CSS_CAPABILITIES.oklch).toMatchObject({ maturity: 'stable', requirements: ['color-level-4'] })
    expect(VANE_CSS_CAPABILITIES.contrastColor).toMatchObject({ maturity: 'experimental', fold: 'fallback-plus-enhancement' })
  })
