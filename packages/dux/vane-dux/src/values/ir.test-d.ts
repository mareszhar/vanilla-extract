...
describe('typed CSS value contracts', () => {
  it('keeps the common interface context-bound while compatibility helpers remain usable', () => {
    const neutral = null as unknown as VaneSelfValue<'length'>
    // @ts-expect-error — context-free `.css` is not part of the common contract
    void neutral.css

    expectTypeOf(length.rem(2)).toExtend<VaneCssValue<`${number}rem`, 'length'>>()
    expectTypeOf(angle.deg(45)).toExtend<VaneSelfValue<'angle'>>()
  })
  ...
  it('infers custom-property types and fallback types', () => {
    ...
    expectTypeOf(typed.$var()).toExtend<VaneValue<'length'>>()
    expectTypeOf(inferred).toExtend<VaneValue<'number'>>()
  })

  it('propagates math results and rejects known incompatible comparisons', () => {
    expectTypeOf(calc(length.px(2)).multiply(cssNumber(3))).toExtend<import('@mszr/vane-dux').VaneCalc<'length'>>()
    expectTypeOf(calc(length.px(2)).divide(length.px(1))).toExtend<import('@mszr/vane-dux').VaneCalc<'number'>>()
    expectTypeOf(min(length.px(2), percent(30))).toExtend<import('@mszr/vane-dux').VaneMathValue<'length-percentage'>>()
