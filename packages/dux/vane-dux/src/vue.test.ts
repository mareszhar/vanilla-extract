/**
 * The runtime plane for the Vue overlay ([dux-spec-vue.md §1–2]): `usePorts`
...
import { propsOf, useAnatomy, usePorts } from '@mszr/vane-dux/vue'
...
describe('usePorts', () => {
  it('a thunk is reactive: the source drives the merged fragment', () => {
    ...
    const style = usePorts(() => fraction.set(value.value))
  ...
  it('the thunk\'s array is the merge — falsy entries skipped, no ports() wrapper', () => {
    ...
    const style = usePorts(() => [fraction.set(0.5), expanded.value && width.set('50%')])
  ...
  it('plain fragments bind statically', () => {
    ...
    expect(usePorts(fraction.set(1)).value).toEqual({ [fraction.name]: 1 })
    expect(usePorts([fraction.set(1), width.set('2rem')]).value)
