...
const LEAK = /ColorValue\b|ContrastValue\b|TokenNode\b|RuleWalker\b|VaneArm\b/
...
describe('the authoring shape', () => {
  ...
  it('rule keys autocomplete with conditions beside properties', () => {
    const result = project.query`${defineSystem}
      void css({ ${cursor} })
    `
    expect(result.completions).toContainCompletions(['open', 'md', 'hover', 'motionOk', 'dark', 'padding', 'layer'])
  })
...
describe('hovers', () => {
  it('the system destructure reads as the bound surface, not an internals wall', () => {
    const result = project.query`${defineSystem}
      void cs${cursor}s
    `
    expect(result.hover).toContain('VaneCssFunction')
