...
describe('environmental axes and declaration contexts', () => {
  ...
  it('validates totality, modes, cases, branch types, and mutable reservations locally', () => {
    ...
    }))).toThrow(/author every mode.*missing: dark/)
  ...
  it('records axis order, arm locality, and resolved token contexts for inspection', () => {
    ...
    expect(token).toMatchObject({
      kind: 'token',
      emission: [
        { kind: 'base', root: ':root', layer: 'vane.tokens.base' },
        {
          kind: 'axis',
          axis: 'density',
          mode: 'compact',
          ...
          placement: 'root',
          priority: 20,
          layer: 'vane.tokens.axes.density',
        },
      ],
    })
  })

  it('uses VaneError for escaped invalid engine/token composition', () => {
    ...
    expect(() => emit(() => (two.createSystem as any)({ tokens: module }))).toThrowError(VaneError)
  })
})
