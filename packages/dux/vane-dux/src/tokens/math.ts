 ...
 * ([dux-spec-tokens.md §2]) — so lightening is `l + delta` in oklch because the
...
export function parseColor(css: string): VaneOklch | undefined {
...
export function formatOklch({ l, c, h, alpha }: VaneOklch): string {
...
export function mixOklch(a: VaneOklch, b: VaneOklch, amount: number): VaneOklch {
...
function oklchOf({ l, c, h, alpha }: VaneOklch) {
  return { mode: 'oklch' as const, l, c, h, alpha }
}

function srgbChannels(color: VaneOklch): [number, number, number] {
 ...
 */
export function apcaContrast(text: VaneOklch, background: VaneOklch): number {
...
export function wcagContrast(a: VaneOklch, b: VaneOklch): number {
...
export const white: VaneOklch = { l: 1, c: 0, h: 0 }
export const black: VaneOklch = { l: 0, c: 0, h: 0 }

export interface VaneContrastPick {
  keyword: 'white' | 'black'
  color: VaneOklch
  lc: number
}
...
export function pickLegible(target: VaneOklch): VaneContrastPick {
