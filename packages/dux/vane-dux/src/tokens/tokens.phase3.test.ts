import { createEngine, defineCssSupportTarget, VaneError } from '@mszr/vane-dux'
import { restoreToken } from '@mszr/vane-dux/runtime'
import { emit } from '@test'
import { describe, expect, it } from 'vitest'

describe('canonical token traits and handles', () => {
  ...
  it('represents no-default bases and exact authored branches without sentinels', () => {
    ...
    expect(ds.t.fill.$name).toBe('--vane-fill')
  ...
  it('projects modules and arbitrary resolved selections with final names', () => {
    ...
    expect(() => ds.tokensOf(foreign)).toThrowError(VaneError)
  })
  ...
  it('applies engine token defaults once to shorthand and omitted config fields', () => {
    ...
    expect(css).not.toContain('--vane-space-sm')
    expect(css).not.toContain('--vane-space-md')
  })
