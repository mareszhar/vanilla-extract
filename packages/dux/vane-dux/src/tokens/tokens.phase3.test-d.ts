...
describe('canonical Phase-3 token types', () => {
  ...
  it('exposes only authored branches and the canonical `$` vocabulary', () => {
    ...
    // @ts-expect-error — the legacy public token name is not on canonical handles
    void ds.t.color.brand.name
    // @ts-expect-error — old token modes are not on the canonical handle model
