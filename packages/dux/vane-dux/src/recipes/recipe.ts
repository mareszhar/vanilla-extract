/**
 * `recipe()` — variants compress state ([dux-spec-recipes.md §1],
 * [dux-patterns.md §7]). Per-arm emission plus a tiny lookup table: each
...
import type { VaneCompiled } from '../css/rule'
...
export function bindRecipe(system: VaneSystemContext) {
  return (options: VaneRecipeOptionsLoose, debugId?: string): VaneRecipe<Record<string, unknown>> => {
    ...
    const variantArms: Record<string, Record<string, VaneCompiled>> = {}
    ...
    const runtime: VaneRecipeRuntime = {
