 ...
 * ([dux-patterns.md §7]). Shared by the build-time factories (which emit the
...
import type { VaneAnatomy, VaneAnatomyRuntime, VaneRecipe, VaneRecipeRuntime } from './types'
 ...
 * never half-styles a component silently ([dux-spec-recipes.md §4]).
 */
function choose(
  ...
  if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'production') {
    ...
    if (!warned.has(key)) {
      warned.add(key)
      console.warn(`[vane] ${runtime.name ?? 'recipe'}: ${axis} got ${JSON.stringify(raw)} — valid values: ${Object.keys(values).join(', ')}`)
    }
  }
...
function selectionOf(
  runtime: Pick<VaneRecipeRuntime, 'name' | 'defaults'> & { variants: Record<string, Record<string, unknown>>, toggles: Record<string, unknown> },
 ...
 */
export function createRecipeHandle(runtime: VaneRecipeRuntime): VaneRecipe<Record<string, unknown>> {
  ...
  return Object.assign(resolve, describe(runtime), {
    toString: () => runtime.base,
  }) as unknown as VaneRecipe<Record<string, unknown>>
}
 ...
 */
export function createAnatomyHandle(runtime: VaneAnatomyRuntime): VaneAnatomy<string, Record<string, unknown>> {
  const warned = new Set<string>()
  ...
  return Object.assign(resolve, describe(runtime), {
    parts: runtime.parts,
  }) as unknown as VaneAnatomy<string, Record<string, unknown>>
}
