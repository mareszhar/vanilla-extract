 ...
 * ([dux-patterns.md §10], [dux-spec-recipes.md]).
 */
...
/** The spec button ([dux-spec-recipes.md §1]) over the Prism system. */
function specButton() {
...
describe('published ports', () => {
  it('rides the recipe: one import gives classes and the style API', () => {
    ...
    expect(returned.button.ports.paddingX.set('24px')).toEqual({
  ...
  it('a static set inside a consumer rule compiles away entirely', () => {
    const { css: emitted, returned } = emit(() => {
      ...
      return system.css({ display: 'flex', ...button.ports.paddingX.set(system.t.space.lg) }, 'toolbar')
    })

    expect(typeof returned).toBe('string')
    expect(emitted).toMatch(/--vane-paddingX__[\w-]+: var\(--vane-space-lg\)/)
  })
...
describe('the untyped edge', () => {
  ...
  it('an undeclared value warns once with the valid set and falls back to the default', () => {
    ...
    expect(warn).toHaveBeenCalledWith('[vane] button: intent got "brnd" — valid values: brand, ghost, danger')
  })
})
