 ...
 * widened props ([dux-spec-recipes.md §4]) — `VaneProps` inference, compound
...
describe('the call site', () => {
  ...
  it('a wider props object just works — no pick, no wrapper', () => {
    const b = button()
    const props: VaneProps<typeof b> & { disabled?: boolean, href?: string } = {
      intent: 'ghost',
      disabled: true,
    }
  ...
  it('VaneProps collapses to the plain optional object', () => {
    const b = button()
    void b

    expectTypeOf<VaneProps<typeof b>>().toEqualTypeOf<{
      intent?: 'brand' | 'ghost'
      size?: 'sm' | 'md'
      pill?: boolean
    }>()
...
describe('the options object', () => {
  ...
  it('arms are full vane rules — unknown properties die at the key', () => {
    const { recipe } = system()
...
describe('anatomy', () => {
  ...
  it('VaneProps works on anatomies identically', () => {
    const d = dialog()
    void d

    expectTypeOf<VaneProps<typeof d>>().toEqualTypeOf<{ size?: 'sm' | 'lg' }>()
  })
})
