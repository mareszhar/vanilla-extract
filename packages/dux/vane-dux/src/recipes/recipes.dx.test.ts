 ...
 * ([dux-spec-recipes.md §6]) — diagnostics land on the offending key with the
...
const LEAK = /ColorValue\b|ContrastValue\b|TokenNode\b|RuleWalker\b|VaneArm\b|VaneRecipeRuntime\b/
...
describe('hovers', () => {
  it('vaneProps collapses to the plain optional object — no internals wall', () => {
    const result = project.query`${defineButton}
      import type { VaneProps } from '@mszr/vane-dux'
      type ButtonPro${cursor}ps = VaneProps<typeof button>
    `
