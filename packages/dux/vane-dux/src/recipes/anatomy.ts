/**
 * `anatomy()` — the recipe pattern applied to parts ([dux-spec-recipes.md §3],
 * [dux-patterns.md §7]). Same options grammar as `recipe` with one added
...
import type { VaneCompiled, VaneScopedConditionResult } from '../css/rule'
import type { VaneConditionArm } from '../system/conditions'
import type { VaneAnatomy, VaneAnatomyRuntime } from './types'
...
type PartArms = Record<string, VaneCompiled>

export function bindAnatomy(system: VaneSystemContext) {
  return (options: VaneAnatomyOptionsLoose, debugId?: string): VaneAnatomy<string, Record<string, unknown>> => {
    ...
    const compilePartArms = (arms: unknown, path: string[]): PartArms => {
      ...
      for (const [part, rule] of Object.entries(arms)) {
        if (!parts.includes(part)) {
          ...
          build.diagnostics.push({
            code: 'VANE_ANATOMY_UNKNOWN_PART',
    ...
    const runtime: VaneAnatomyRuntime = {
 ...
 * ([dux-spec-recipes.md §3]). Each of the condition's arms is re-anchored:
...
): (key: string) => VaneScopedConditionResult | undefined {
  return (key) => {
    ...
    if (condition === undefined) {
      ...
      return {
        diagnostic: {
          code: 'VANE_ANATOMY_INVALID_CONDITION',
