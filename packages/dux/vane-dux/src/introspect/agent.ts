...
export interface VaneAgentContext {
  readonly manifestVersion: VaneManifest['version']
  readonly system: {
...
export function buildAgentContext(manifest: VaneManifest): VaneAgentContext {
...
export function generateAgentContext(manifest: VaneManifest): string {
  const context = buildAgentContext(manifest)
  const lines = [
    '# vane-dux system context',
    '',
