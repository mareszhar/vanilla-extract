import { createEngine, defineEnginePlugin, exportDesignTokens, importDesignTokens } from '@mszr/vane-dux'
...
describe('dTCG interchange', () => {
  ...
  it('round-trips portable authored traits, aliases, axes, and reservations', () => {
    ...
    expect(document.$extensions['com.mszr.vane-dux']).toMatchObject({ version: 1, mode: 'authored' })

    const imported = importDesignTokens(document, { engine: de })
  ...
  it('orders authored imports by dependencies that exist only inside branches', () => {
    ...
    const extension = document.$extensions['com.mszr.vane-dux']
    const reordered = {
      ...document,
      $extensions: {
        ...document.$extensions,
    ...
    const imported = importDesignTokens(reordered, { engine: de })
  ...
  it('exports an unfinished module only with its explicit engine context', () => {
    ...
    expect(() => exportDesignTokens(module)).toThrow('needs options.engine')
    expect(exportDesignTokens(module, { engine: de, prefix: 'portable', root: '#widget' })).toMatchObject({
  ...
  it('fails strict authored export for an opaque plugin without a codec', () => {
    ...
    expect(lossy.$extensions['com.mszr.vane-dux'].tokens['space.mystery'].lossy).toBe(true)
  })
  ...
  it('lets a public plugin codec make opaque semantics losslessly portable', () => {
    ...
    expect(document.$extensions['com.mszr.vane-dux'].tokens['space.mystery'].val.codec).toMatchObject({
      id: 'org.example.codec',
      version: 1,
    })
    expect(document.$extensions['com.mszr.vane-dux'].tokens['space.mystery'].branches[0].val.codec).toMatchObject({
      id: 'org.example.codec',
      version: 1,
    })
    expect(document.$extensions['com.mszr.vane-dux'].tokens['space.mystery'].branches[1].val.codec).toMatchObject({
      id: 'org.example.codec',
      version: 1,
    })
