import type { VaneEngine } from '../engine/createEngine'
import type { VaneTokenRecord } from '../internal/inspect'
import type { VaneDtcgCodec, VaneInterchangeSystem, VaneJsonValue } from '../internal/interchange'
import type { VaneGraphInput, VaneTokenModule, VaneTokenPolicy } from '../tokens/types'
...
export const VANE_DTCG_EXTENSION = 'com.mszr.vane-dux' as const
export const VANE_DTCG_EXTENSION_VERSION = 1 as const

export type VaneDtcgDocument = Readonly<Record<string, unknown>>
export type VaneDtcgExportMode = 'resolved' | 'authored'

export interface VaneDtcgExportOptions {
  readonly mode?: VaneDtcgExportMode
  ...
  readonly engine?: VaneEngine<any, any, any>
  readonly prefix?: string
  readonly root?: string
}

export interface VaneDtcgImportOptions {
  /** Defaults to a zero-config engine for ordinary standard DTCG documents. */
  readonly engine?: VaneEngine<any, any, any>
  readonly strict?: boolean
...
export interface VaneDtcgAuthoredExtension {
  readonly version: typeof VANE_DTCG_EXTENSION_VERSION
  ...
  readonly tokens: Readonly<Record<string, VaneDtcgAuthoredToken>>
}

export interface VaneDtcgAuthoredToken {
  ...
  readonly val: VaneDtcgEncodedValue
  readonly branches: readonly {
    readonly address: VaneTokenRecord['semantic']['branches'][number]['address']
    readonly val: VaneDtcgEncodedValue
  }[]
  readonly register?: VaneTokenRecord['semantic']['registration']
...
export interface VaneDtcgEncodedValue {
  readonly css: string | number | null
  readonly dependencies: readonly { readonly path: string, readonly name?: `--${string}` }[]
  readonly codec?: { readonly id: string, readonly version: string | number, readonly payload: VaneJsonValue }
}
...
/** Export either a standard environment snapshot or a Vane-authored round-trip document. */
export function exportDesignTokens(
  source: object,
  options: VaneDtcgExportOptions = {},
): VaneDtcgDocument {
  ...
  for (const [axis, mode] of Object.entries(environment)) {
    ...
    if (!definition || !(mode in definition.modes))
      throw new TypeError(`[vane] DTCG export environment selects unknown axis mode '${axis}.${mode}'`)
  }
  ...
  for (const token of records) {
    try {
      ...
      try {
        ...
        // authored contract still lives in the Vane extension below.
        continue
      }
      setToken(document, token.path.split('.'), {
        $value: css,
  ...
  if (mode === 'authored') {
    const authored: Record<string, VaneDtcgAuthoredToken> = {}
    for (const token of records) {
      if (token.semantic.portability.status === 'nonportable' && strict)
        throw new TypeError(`[vane] ${token.path} is not losslessly portable: ${token.semantic.portability.reason}`)
    ...
    document.$extensions = {
      ...existing,
      [VANE_DTCG_EXTENSION]: {
        version: VANE_DTCG_EXTENSION_VERSION,
        mode: 'authored',
        system: {
        ...
        tokens: authored,
      } satisfies VaneDtcgAuthoredExtension,
    }
  }
...
/** Import a standard DTCG document, or restore the richer authored Vane extension. */
export function importDesignTokens(
  document: unknown,
  options: VaneDtcgImportOptions = {},
): VaneTokenModule<VaneGraphInput, VaneTokenPolicy> {
  if (!isPlainObject(document))
    throw new TypeError('[vane] importDesignTokens() needs one DTCG document object')
...
function exportSource(source: object, options: VaneDtcgExportOptions): ExportSource {
  if (VANE_SYSTEM_INTERCHANGE in source) {
    const interchange = (source as VaneInterchangeSystem)[VANE_SYSTEM_INTERCHANGE]
    return { graph: interchange.graph, codecs: interchange.codecs }
  }
  if (!isTokenBuilder(source))
    throw new TypeError('[vane] exportDesignTokens() needs a bound system or unfinished token module')
  if (!options.engine)
    throw new TypeError('[vane] exporting an unfinished token module needs options.engine')
  const privateEngine = enginePrivate(options.engine as any)
  const tokens = finalizeTokenModule(source, {
    prefix: options.prefix ?? 'vane',
...
function authoredExtension(document: Record<string, unknown>): VaneDtcgAuthoredExtension | undefined {
  const extensions = document.$extensions
  if (!isPlainObject(extensions))
    return undefined
  const extension = extensions[VANE_DTCG_EXTENSION]
  if (!isPlainObject(extension))
    return undefined
  if (extension.version !== VANE_DTCG_EXTENSION_VERSION || extension.mode !== 'authored' || !isPlainObject(extension.tokens))
    throw new TypeError(`[vane] unsupported ${VANE_DTCG_EXTENSION} authored extension version`)
  return extension as unknown as VaneDtcgAuthoredExtension
}

function importAuthored(
  extension: VaneDtcgAuthoredExtension,
  document: Record<string, unknown>,
  engine: VaneEngine<any, any, any>,
  options: VaneDtcgImportOptions,
): VaneTokenModule<VaneGraphInput, VaneTokenPolicy> {
  const actualAxisOrder = enginePrivate(engine as any).axes.order
  if (actualAxisOrder.join('\0') !== extension.system.axisOrder.join('\0')) {
    throw new TypeError(
      `[vane] authored DTCG axis order (${extension.system.axisOrder.join(', ') || 'none'}) does not match the import engine (${actualAxisOrder.join(', ') || 'none'})`,
    )
  }
  ...
  for (const path of ordered) {
    ...
    if (token.lossy && (options.strict ?? true))
      throw new TypeError(`[vane] authored token '${path}' was exported lossily and cannot be imported in strict mode`)
  ...
  return module as VaneTokenModule<VaneGraphInput, VaneTokenPolicy>
}
...
function importStandard(
  ...
  if (blocked && !options.resolveExternal)
    throw new TypeError(`[vane] external DTCG reference '${blocked.value}' is disabled; pass resolveExternal explicitly`)
  ...
  for (const path of ordered) {
    ...
    module = module.derive((t: object) => {
      ...
      const metadata: Record<string, VaneJsonValue> = {}
      if (token.extensions !== undefined)
        metadata.dtcgExtensions = token.extensions as VaneJsonValue
      if (token.groupExtensions.length > 0)
        metadata.dtcgGroupExtensions = token.groupExtensions as unknown as VaneJsonValue
      if (token.alias)
      ...
      return nested(path, configuredByType(engine, vaneType(token.type), config))
    })
  }
  return module as VaneTokenModule<VaneGraphInput, VaneTokenPolicy>
}
...
function collectStandard(
  ...
  if (isPlainObject(group.$root) && '$value' in group.$root) {
    if (path.length === 0)
      throw new TypeError('[vane] a document-level DTCG $root token has no Vane token path')
    const siblings = Object.keys(group).filter(key => !key.startsWith('$'))
    if (siblings.length > 0) {
      throw new TypeError(
        `[vane] DTCG group '${path.join('.')}' has both a $root token and children; Vane token paths cannot be a leaf and group simultaneously`,
      )
    }
  ...
  for (const [key, value] of Object.entries(group)) {
    ...
    if (!isPlainObject(value))
      throw new TypeError(`[vane] DTCG group '${[...path, key].join('.')}' must be an object`)
    const next = [...path, key]
...
function selectedCss(token: VaneTokenRecord, axisOrder: readonly string[], environment: Readonly<Record<string, string>>): string | number {
...
function standardCarrier(
  ...
  records: readonly VaneTokenRecord[],
...
function exactTokenAlias(text: string, token: VaneTokenRecord, records: readonly VaneTokenRecord[]): string | undefined {
...
function dtcgType(type: VaneCssDataType): string | undefined {
...
function vaneType(type: string | undefined): VaneCssDataType {
  if (type === 'number')
    return 'number'
  if (type === 'dimension')
    return 'length'
...
function standardCss(type: string | undefined, value: unknown, options: VaneDtcgImportOptions): unknown {
  if (typeof value === 'string' && externalReference(value)) {
    if (!options.resolveExternal)
      throw new TypeError(`[vane] external DTCG reference '${value}' is disabled; pass resolveExternal explicitly`)
    const resolved = options.resolveExternal(value)
    if (isThenable(resolved))
      throw new TypeError('[vane] resolveExternal must return synchronously; resolve asynchronous documents before importing them')
    return resolved
  }
  if (type === 'number') {
    if (typeof value !== 'number' || !Number.isFinite(value))
      throw new TypeError('[vane] a DTCG number token needs a finite numeric $value')
    return value
  }
  if (type === 'dimension' || type === 'duration') {
    if (!isPlainObject(value) || typeof value.value !== 'number' || typeof value.unit !== 'string')
      throw new TypeError(`[vane] a DTCG ${type} token needs { value, unit }`)
    if (type === 'dimension' && value.unit !== 'px' && value.unit !== 'rem')
      throw new TypeError('[vane] DTCG dimensions support only px and rem')
    if (type === 'duration' && value.unit !== 'ms' && value.unit !== 's')
      throw new TypeError('[vane] DTCG durations support only ms and s')
    return `${value.value}${value.unit}`
  }
  if (type === 'color') {
    if (!isPlainObject(value) || typeof value.colorSpace !== 'string' || !Array.isArray(value.components)
      || value.components.length !== 3) {
      throw new TypeError('[vane] a DTCG color token needs { colorSpace, components[3], alpha? }')
    }
    if (!value.components.every(component => (typeof component === 'number' && Number.isFinite(component)) || component === 'none'))
      throw new TypeError('[vane] DTCG color components must be finite numbers or "none"')
    const [first, second, third] = value.components
    const alpha = value.alpha ?? 1
    if (typeof alpha !== 'number' || !Number.isFinite(alpha))
      throw new TypeError('[vane] DTCG color alpha must be a finite number')
    ...
    throw new TypeError(`[vane] unsupported DTCG color space '${value.colorSpace}'`)
  }
  if (options.strict ?? true)
    throw new TypeError(`[vane] DTCG type '${type ?? 'missing'}' is not supported by the standard importer`)
  return typeof value === 'string' || typeof value === 'number' ? value : JSON.stringify(value)
}

function decodeValue(
  encoded: VaneDtcgEncodedValue,
  type: VaneCssDataType,
  tree: object,
  engine: VaneEngine<any, any, any>,
): unknown {
  ...
  if (encoded.codec) {
    ...
    if (!codec)
      throw new TypeError(`[vane] authored value needs DTCG codec '${encoded.codec.id}@${encoded.codec.version}'`)
    return codec.decode({
...
function encodedValue(
  ...
  token: VaneTokenRecord,
  expression: VaneTokenRecord['semantic']['expression'] | undefined,
  codecs: readonly VaneDtcgCodec[],
...
function configuredByType(engine: VaneEngine<any, any, any>, type: VaneCssDataType, config: Record<string, unknown>): unknown {
  if ('val' in config)
...
function codecForExpression(
  expression: VaneTokenRecord['semantic']['expression'],
  codecs: readonly VaneDtcgCodec[],
): VaneDtcgCodec | undefined {
...
function preservedExtensionsAt(
  ...
  if (document.$extensions !== undefined) {
    const rootExtensions = isPlainObject(document.$extensions)
      ? Object.fromEntries(Object.entries(document.$extensions).filter(([name]) => name !== VANE_DTCG_EXTENSION))
      : document.$extensions
...
function restoreGroupExtensions(document: Record<string, unknown>, records: readonly VaneTokenRecord[]): void {
  ...
  if (value.startsWith('#/')) {
    ...
    for (const part of parts) {
      if (!isPlainObject(current) || !(part in current))
        throw new TypeError(`[vane] unresolved DTCG JSON Pointer '${value}'`)
...
function topological(entries: readonly { path: string, dependencies: readonly string[] }[]): string[] {
  ...
  for (const entry of entries) {
    ...
    if (unknown !== undefined)
      throw new TypeError(`[vane] DTCG token '${entry.path}' references unknown token '${unknown}'`)
  }
  ...
  if (result.length !== entries.length) {
    ...
    throw new TypeError(`[vane] DTCG alias/dependency cycle: ${pending.join(' → ')}`)
  }
  return result
}
...
function tokenAt(tree: object, path: string): unknown {
  ...
  for (const part of path.split('.')) {
    ...
    if (current === undefined)
      throw new TypeError(`[vane] DTCG reference '${path}' does not name an imported token`)
  }
  return current
}
...
function setToken(document: Record<string, unknown>, path: readonly string[], token: object): void {
  for (const segment of path) {
    if (segment.startsWith('$') || /[.{}]/.test(segment))
      throw new TypeError(`[vane] token segment '${segment}' cannot be represented as a DTCG name`)
  }
...
function exportError(path: string, error: unknown): TypeError {
  return new TypeError(`[vane] cannot export '${path}' as standard DTCG: ${error instanceof Error ? error.message : String(error)}`)
}

function assertJson(value: unknown, label: string, seen = new Set<object>()): asserts value is VaneJsonValue {
  if (value === null || typeof value === 'string' || typeof value === 'boolean')
    return
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      throw new TypeError(`[vane] ${label} contains a non-finite number`)
    return
  }
  if (typeof value !== 'object')
    throw new TypeError(`[vane] ${label} is not JSON-safe`)
  if (seen.has(value))
    throw new TypeError(`[vane] ${label} contains a cycle`)
  seen.add(value)
  if (Array.isArray(value)) {
    ...
    if (!isPlainObject(value))
      throw new TypeError(`[vane] ${label} contains a non-plain object`)
