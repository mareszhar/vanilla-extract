/**
 * The runtime plane for the preset conveniences ([dux-spec-preset.md §3–6]):
...
describe('a11y helpers', () => {
  it('focusRing is a :focus-visible-scoped fragment, token-driven', () => {
    ...
    const brandish = { var: 'var(--vane-color-brand)' } as const
    expect(focusRing({ color: brandish, width: '3px' }).focusVisible.outline)
      .toBe('3px solid var(--vane-color-brand)')
  })
...
describe('layout patterns', () => {
  ...
  it('the documented CSS comes out the back', () => {
    ...
    expect(css).toContain('gap: var(--vane-space-md)')
