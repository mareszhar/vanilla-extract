...
describe('canonical engine types', () => {
  ...
  it('rejects a plugin whose required earlier namespace is absent', () => {
    ...
    const expansion: VaneEnginePlugin<{
  ...
  it('rejects legacy finalized graphs and reserved extension names at the cursor', () => {
    ...
    de.extend(() => ({
      // @ts-expect-error css is reserved by system surface v1
      css: {},
    }))
    de.extend(() => ({
