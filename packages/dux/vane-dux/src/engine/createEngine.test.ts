...
describe('the canonical design engine', () => {
  it('is a complete zero-config authoring environment with semantic identity', () => {
    ...
    expect(() => legacy.serialize(legacy.calc('1rem').add('2px'))).toThrow(/no-modern-values/)
    expect(Object.isFrozen(first)).toBe(true)
  })

  it('publishes one closed system/constructor reservation contract', () => {
    expect(VANE_SYSTEM_SURFACE_VERSION).toBe(1)
    expect(VANE_SYSTEM_MEMBERS).toContain('css')
    expect(VANE_SYSTEM_MEMBERS).toContain('explain')
    expect(VANE_BUILTIN_CONSTRUCTOR_NAMES).toContain('length')
    expect([...VANE_BUILTIN_CONSTRUCTOR_NAMES].sort()).toEqual(
      Object.keys(createCoreConstructors('px')).sort(),
    )
    expect(Object.isFrozen(VANE_SYSTEM_MEMBERS)).toBe(true)
  ...
  it('composes equivalent and parent-engine modules but rejects changed semantics', () => {
    ...
    expect(() => changed.defineTokens().compose(colors)).toThrowError(VaneError)
  ...
  it('snapshots module structure and keeps every composition link immutable', () => {
    ...
    expect(css).toContain('--vane-color-brand: #635bff')
    expect(css).not.toContain('--vane-color-brand: #f00')
  })
