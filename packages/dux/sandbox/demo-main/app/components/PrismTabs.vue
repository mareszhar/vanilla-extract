...
import { tabs } from './PrismTabs.style'

const props = defineProps<{ items: { label: string, content: string }[] }>()

const active = ref(0)
const parts = useAnatomy(tabs)
const id = useId()
...
function onKeydown(event: KeyboardEvent): void {
  if (event.key === 'ArrowRight') moveFocus(active.value + 1)
  else if (event.key === 'ArrowLeft') moveFocus(active.value - 1)
  else if (event.key === 'Home') moveFocus(0)
  else if (event.key === 'End') moveFocus(props.items.length - 1)
  else return
